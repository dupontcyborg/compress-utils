...
language (C / WASM / Python / Go / Rust) from a multi-language results file, at
one representative level + mode so each group compares the same codec across
languages.
...
# Rust takes the graphite of its logo rather than its orange, which at bar sizes
# is indistinguishable from the JS/TS tone.
...
def main() -> None:
    ...
    # whatever subset of the five a run covered.
